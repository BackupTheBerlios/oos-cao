<?php
/* ----------------------------------------------------------------------
   $Id: process.php,v 1.3 2006/07/26 05:02:44 r23 Exp $
  ...
  include OOS_LANGUAGES . $sLanguage . '/' . $sMp . '_' . $sFile . '.php';
  include OOS_FUNCTIONS . 'function_address.php';
...
// if the customer is not logged on, redirect them to the login page
  ...
  if (!isset($_SESSION['sendto'])) {
    oos_redirect(oos_link($aModules['checkout'], $aFilename['checkout_payment'], '', 'SSL'));
  }
...
// avoid hack attempts during the checkout procedure by checking the internal cartID
  if (isset($_SESSION['cart']->cartID) && isset($_SESSION['cartID'])) {
    if ($_SESSION['cart']->cartID != $_SESSION['cartID']) {
      oos_redirect(oos_link($aModules['checkout'], $aFilename['checkout_shipping'], '', 'SSL'));
    }
  }

// load selected payment module
  include OOS_CLASSES . 'class_payment.php';
  $payment_modules = new payment($_SESSION['payment']);

// load the selected shipping module
  include OOS_CLASSES . 'class_shipping.php';
  $shipping_modules = new shipping($_SESSION['shipping']);

  include OOS_CLASSES . 'class_order.php';
  $order = new order;

// load the before_process function from the payment modules
  $payment_modules->before_process();

  include OOS_CLASSES . 'class_order_total.php';
    ...
    if (STOCK_LIMITED == 'true') {
      if (DOWNLOAD_ENABLED == 'true') {
        $stock_result_raw = "SELECT products_quantity, pad.products_attributes_filename 
                             ...
                             WHERE p.products_id = '" . intval(oos_get_product_id($order->products[$i]['id'])) . "'";
        ...
        $sql = "SELECT products_quantity
                ...
                WHERE products_id = '" . intval(oos_get_product_id($order->products[$i]['id'])) . "'";
...
// Update products_ordered (for bestsellers list)
    ...
    $sql_data_array = array('orders_id' => $insert_id, 
                            'products_id' => oos_get_product_id($order->products[$i]['id']), 
...
//------insert customer choosen option to order--------
    ...
    if (isset($order->products[$i]['attributes'])) {
      ...
      for ($j=0, $n2=count($order->products[$i]['attributes']); $j<$n2; $j++) {
        ...
        $products_ordered_attributes .= "\n\t" . $attributes_values['products_options_name'] . ' ' . oos_decode_special_chars($order->products[$i]['attributes'][$j]['value']);
      }
    }
//------insert customer choosen option eof ----
    $total_weight += ($order->products[$i]['qty'] * $order->products[$i]['weight']);
    $total_tax += oos_calculate_tax($total_products_price, $products_tax) * $order->products[$i]['qty'];
    $total_cost += $total_products_price;

    $products_ordered .= $order->products[$i]['qty'] . ' x ' . $order->products[$i]['name'] . ' (' . $order->products[$i]['model'] . ') = ' . $oCurrencies->display_price($order->products[$i]['final_price'], $order->products[$i]['tax'], $order->products[$i]['qty']) . $products_ordered_attributes . "\n";
  }
  ...
  $email_order = STORE_NAME . "\n" .
                 ...
                 $aLang['email_text_invoice_url'] . ' ' . oos_link($aModules['account'], $aFilename['account_history_info'], 'order_id=' . $insert_id, 'SSL', false) . "\n" .
  ...
  if (!isset($_SESSION['man_key'])) {
    oos_mail($order->customer['firstname'] . ' ' . $order->customer['lastname'], $order->customer['email_address'], $aLang['email_text_subject'], nl2br($email_order), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, '');
  }
